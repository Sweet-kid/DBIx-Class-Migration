...
sub _build_test_postgresql {
  ...
  unless($auto_start) {
    open ( my $pid_fh, '<', catdir($base_dir, 'data','postmaster.pid')) ||
      $self->log->error( "Can't open PID file" );
    my @lines = <$pid_fh>;
    close ($pid_fh);
    $config{port} = $lines[3];
  }
  ...
  if(my $testdb = Test::PostgreSQL->new(%config)) {
    ...
    $self->log->error( $Test::PostgreSQL::errstr );
  }
}
...
sub _write_start {
  ...
  open( my $fh, '>', catfile($bin, 'start'))
    || $self->log->error( "Cannot open $bin/start: $!" );
...
sub _write_stop {
  ...
  open( my $fh, '>', catfile($bin, 'stop'))
    || $self->log->error( "Cannot open $bin/stop: $!" );
...
sub _write_use {
  ...
  open( my $fh, '>', catfile($bin, 'use'))
    || $self->log->error( "Cannot open $bin/use: $!" );
...
sub _write_dump {
  ...
  open( my $fh, '>', catfile($bin, 'dump'))
    || $self->log->error( "Cannot open $bin/dump: $!" );
...
sub _write_config {
  ...
  open( my $fh, '>', catfile($bin, 'config'))
    || $self->log->error( "Cannot open $bin/config $!" );
...
sub make_sandbox {
  ...
  if($self->test_postgresql) {
    ...
    $self->log->error( "can't start a postgresql sandbox" );
  }
}
